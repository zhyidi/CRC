...
from dataset_mT5 import mT5Dataset
...
def main():
    ...
    model_dir += "_stage" + str(args.stage)
    ...
    # prepare data
    dataset = load_data(args.stage, tokenizer, logger, args.test_path, args.cache_dir, 
        data_partition="test", use_tcp=args.use_tcp, tcp_path=args.tcp_path)

    test_dataset = mT5Dataset(dataset, tokenizer, max_seq_len=args.max_seq_len)
    ...
    output_dir += "_stage" + str(args.stage)
    ...
    with open(output_path, 'w', encoding="utf-8") as f:
        ...
        with torch.no_grad():
            for batch in tqdm(test_loader):
                input_ids, _ = tuple(input_tensor.to(args.device) for input_tensor in batch)
