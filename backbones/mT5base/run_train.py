...
from transformers import MT5ForConditionalGeneration, MT5Tokenizer, AdamW, WEIGHTS_NAME, CONFIG_NAME
from torch.utils.data import DataLoader
from data_utils import load_data, NEW_ADD_TOKENS
from dataset_mT5 import mT5Dataset, IGNORE_INDEX
...
def train():
    ...
    parser.add_argument("--stage", type=int, default=None, help="train stage: 1 or 2 or 3")         
    ...
    train_data = load_data(args.stage, tokenizer, logger, args.train_path, args.cache_dir, 
        data_partition="train", use_tcp=args.use_tcp)
    valid_data = load_data(args.stage, tokenizer, logger, args.valid_path, args.cache_dir, 
        data_partition="valid", use_tcp=args.use_tcp)
    
    train_dataset = mT5Dataset(train_data, tokenizer, max_seq_len=args.max_seq_len)
    valid_dataset = mT5Dataset(valid_data, tokenizer, max_seq_len=args.max_seq_len)
    ...
    def update(engine, batch):
        ...
        model.train()
        lm_loss = model(input_ids=input_ids, labels=lm_labels).loss
    ...
    def inference(engine, batch):
        ...
        with torch.no_grad():
            input_ids, lm_labels = tuple(input_tensor.to(args.device) for input_tensor in batch)
            lm_logits = model(input_ids=input_ids, labels=lm_labels).logits
