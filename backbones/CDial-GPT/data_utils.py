...
def convert_data(data_partition, stage, fp, extract_kg=False, tcp_path=None):
    ...
    with open(fp, 'r', encoding='utf-8') as fr:
        for idx, line in enumerate(fr):
            ...
            if stage==2 and data_partition=="train":
                resp_str += " ###rq:" + sample["response_quality"]
                resp_str += " #reason:" + sample["reason"]
                resp_str += " #how:" + sample["How_to_be_better"]
            ...
            input_str += BOT
            if stage==2:
                input_str += SEP + "###stage2"
            if stage==3:
                input_str += SEP + "###stage3"
                if "pred_response" in sample:
                    input_str += SEP + "#pre_rsp:" +sample["pred_response"]
                if "response_quality" in sample:
                    input_str += " #rq:" +sample["response_quality"]
                if "reason" in sample:
                    input_str += " #reason:" +sample["reason"]
                if "How_to_be_better" in sample:
                    input_str += " #how:" +sample["How_to_be_better"]
            ...
            resp_tok_str = resp_tok_str.replace("[ S E P ]", SEP)
...
def load_data(stage, tokenizer, logger, dataset_path, cache_dir, data_partition="train", use_tcp=False, tcp_path=None):
    """ Load data from cache or create from raw data."""
    if use_tcp:
        cache_dir = cache_dir + '_w_TCP'
    cache_dir = cache_dir + '_stage' + str(stage)
    ...
    if os.path.exists(cache_path):
        ...
        if use_tcp:
            if data_partition == "test":
                ...
                data = convert_data(data_partition, stage, fp=dataset_path, extract_kg=True, tcp_path=tcp_path)
            else:
                logger.info("Prepare train/valid data for TCP-enhanced generation.")
                # prepare train/valid data for TCP-enhanced fine-tuning
                data = convert_data(data_partition, stage, fp=dataset_path, extract_kg=True)
        else:
            # prepare data for CDial-GPT fine-tuning
            data = convert_data(data_partition, stage, fp=dataset_path)  
